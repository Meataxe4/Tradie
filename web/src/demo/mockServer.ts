...
function classify(desc: string): AnyMap {
  ...
  if (/(water|leak|leaking|drip|damp|moisture|flood|wet\b|overflow)/i.test(t))
    return { verdict: "NEEDS_LICENSED_PRO", category: "plumbing_water", regulated_domains: ["plumbing_water"], safety_flags: ["none"], recommended_trade: "plumber", required_licence_class: "Plumbing contractor licence", diy_guidance: null, why_pro_needed: "Water where it shouldn't be is licensed plumbing work — it needs a licensed plumber.", job_spec: spec("Water problem — source to be found", "Water appearing where it shouldn't; a licensed plumber needs to find and fix the source.", ["Water present where it should not be"], ["Have you turned off the water at the mains?"], "urgent"), user_message: "Water where it shouldn't be needs a licensed plumber to find the source.", ...base };
...
export function handleRequest(method: string, path: string, body: any, authHeader?: string): Res {
  ...
  try {
    ...
    if (method === "GET" && seg[0] === "jobs" && seg[2] === "quotes") { const u = need("homeowner"); const job = db.jobs.get(seg[1]!); if (!job || job.homeowner_id !== u.sub) return err(404, "Job not found"); for (const q of quotesForJob(job.id)) if (q.status === "offered" && !q.viewed_at) { q.viewed_at = nowIso(); db.quotes.set(q.id, q); } return ok(quotesForJob(job.id).map(quoteView)); }
